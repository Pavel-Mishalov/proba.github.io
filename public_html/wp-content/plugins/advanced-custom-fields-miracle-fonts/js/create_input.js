(function($){
	...
	$(document).ready(function($) {
		...
		if ( $('.icommon-field-wrapper').length > 0 ) {
			version_num = 4;
		}
	...
	function update_preview( element, selected, version ) {
		if ( version == 4 ) {
			$( '.icommon-live-preview', $(element).parent() ).html( '<i class="' + selected + '"></i>' );
		} else {
			var sibling = $(element).closest('tr').siblings('tr[data-name="fa_live_preview"]');
			$( 'td.acf-input', sibling ).html( '<i class="' + selected + '"></i>' );
		}
	}
