(function($){
	...
	function update_preview( element, selected ) {
		...
		$( '.icommon-live-preview', parent ).html( '<i class="' + selected + '"></i>' );
	}
	...
	if( typeof acf.add_action !== 'undefined' ) {
		...
		acf.add_action('ready append', function( $el ){
			...
			acf.get_fields({ type : 'font-icommon'}, $el).each(function(){
		...
		$(document).live('acf/setup_fields', function(e, postbox){
			...
			$(postbox).find('.field[data-field_type="font-icommon"], .sub_field[data-field_type="font-icommon"]').each(function(){
				initialize_field( $(this) );
			});
