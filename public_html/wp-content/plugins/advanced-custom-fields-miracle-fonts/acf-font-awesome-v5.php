...
class acf_field_font_icommon extends acf_field {
	...
	*/
	...
	function __construct() {	
		...
		$this->name = 'font-icommon';
		$this->label = __('Иконки Проекта');
	...
	function get_icons()
	{
		...
		$project = file_get_contents( dirname( __FILE__ ) . '/icommon.json' );
		...
		$bfa 		= Better_Font_Icommon_Library::get_instance( $args );
		$bfa_icons	= $project->icons;
		$bfa_prefix	= $project->preferences->fontPref->prefix;
		$new_icons	= array(  );
		...
		$iterator = 0;
		foreach ( $bfa_icons as $icon ) {
			$unicode = ( $iterator < 16 ) ? '&#x90' . dechex( $icon->iconIdx ) . ';' : '&#x9' . dechex( $icon->iconIdx ) . ';';
			$new_icons[ $bfa_prefix . $icon->properties->name ] = $icon->properties->name;
		}
	...
	*/
	...
	function render_field( $field )
	{
		...
		// html
		echo '<div class="icommon-field-wrapper">';
		echo '<div class="icommon-live-preview"></div>';
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		wp_enqueue_script('acf-input-font-icommon-edit-input', $this->settings['dir'] . 'js/edit_input.js', array(), $this->settings['version']);
		wp_enqueue_style('acf-input-font-icommon-input', $this->settings['dir'] . 'css/input.css', array(), $this->settings['version']);
		wp_enqueue_style('acf-input-font-icommon-fa', $this->stylesheet, array(), $this->version);
	}
	...
	*/
	...
	function field_group_admin_enqueue_scripts() {
		...
		// register acf scripts
		wp_enqueue_script('font-icommon-create-input', $this->settings['dir'] . 'js/create_input.js', array(), $this->settings['version']);
		wp_enqueue_style('acf-input-font-icommon-input', $this->settings['dir'] . 'css/input.css', array(), $this->settings['version']);
		wp_enqueue_style('acf-input-font-icommon-fa', $this->stylesheet, array(), $this->version);
	}
